...
**内容动图 vs 内置图标动效(别混)**:本节(DOM overlay)只管**内容动图**——LLM/用户给的 GIF / 动画 SVG **文件**。**引擎内置图标 / chrome(copy 图标、loading、装饰)的动效不走这条**:它们应是**矢量 → SDF + 程序化动画**(引擎 anim:旋转 / 脉冲 / `mix` morph,0025 / 0016 / [Plan 10 §4 图标 morph]),canvas 内、缩放锐利、可按 0021 重上色、**零纹理上传、canvas 仍冻结**。内置图标**不做成 GIF / 动画 SVG 文件**(糊、不可重色、不可 morph、毁冻结)。仅当某内置资产无法程序化复刻,才退 DOM overlay。
